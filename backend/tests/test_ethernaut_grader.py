"""Unit tests for the Ethernaut grader.

The grader's `run` function takes a config dict + a player address and
asks `app.web3_client.get_web3()` for an RPC client. We patch that import
in the grader module so no network is touched during the tests.
"""
...
# Tests must be importable without a real .env present. Set required env
# vars BEFORE the grader module loads (it imports get_web3 which reads
...
HELLO = "0x7e0f53981657345b31c59ac44e9c21631ce710c7"   # Ethernaut level 0
FALLBACK = "0x3c34a342b2af5e885fcaa3800db5b205fefa3ffb"  # level 1
...
def _log(level_addr: str, block: int) -> dict:
    return {
        "topics": [
            ethernaut._topic0(),
            _padded(PLAYER),
            _padded("0x" + "1" * 40),  # instance address (don't care)
            _padded(level_addr),
        ],
        "blockNumber": block,
    }
...
def _make_w3(logs: list[dict], head: int = 1_040_000) -> MagicMock:
    """Default `head` keeps the from_block..head window inside a single
    50k-block chunk, so a single `return_value` doesn't bleed across
    paginated calls. Tests that exercise pagination pass head=... explicitly
    and use side_effect instead."""
    w3 = MagicMock()
...
def test_unknown_level_addresses_are_ignored() -> None:
    logs = [_log("0x" + "9" * 40, 1_050_000)]  # not in level_scores
    with patch.object(ethernaut, "get_web3", return_value=_make_w3(logs)):
        result = ethernaut.run(_config(), PLAYER)
    assert result.status == "ok"
    assert result.score == 0
...
def test_caps_at_max_score_then_adds_bonus() -> None:
    # 16 distinct levels worth 1 point each -> raw=16, capped=10, bonus=+4 -> 14
    cfg = _config(
        {
...
def test_below_pass_threshold_does_not_pass() -> None:
    # raw=1 (just Fallback), under pass=7
...
def test_paginates_block_range() -> None:
    """from_block=1_000_000, head=1_120_000, chunk=50_000 -> 3 calls."""
